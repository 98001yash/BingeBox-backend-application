...
public class MovieSpecification {
    ...
    public static Specification<Movie> filterBy(
            String title,
            Set<Long> genreIds,
            Set<Long> actorIds,
        ...
        return (root, query, cb) -> {
            ...
            if (directorIds != null && !directorIds.isEmpty()) {
                Join<Movie, com.company.BingeBox_backend_application.catalog_service.entity.Director> directors = root.join("directors");
                predicates.add(directors.get("id").in(directorIds));
            }
            if (producerIds != null && !producerIds.isEmpty()) {
                Join<Movie, com.company.BingeBox_backend_application.catalog_service.entity.Producer> producers = root.join("producers");
